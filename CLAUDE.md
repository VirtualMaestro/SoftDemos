...
analyzer/instruction candidates distilled from past sessions are in `ANALYZER-RULES.md`.
...
- Simulation (pure C# + DragonECS, `noEngineReferences`, guarded by `ArchitectureTests`):
  `Client.Simulation.Shared` plus one `Client.Simulation.<Feature>` per demo
  (AceOfShadows, MagicWords, PhoenixFlame). Talks to the outside world only through ports:
  shared ones in `Simulation/Shared/Ports`, single-feature ones in that feature's own
  `Ports/` folder (`Simulation/MagicWords/Ports`).
- Adapters: `Client.Adapters.Shared`, `Client.Adapters.Vendor`, `Client.Adapters.Shell`,
  and one `Client.Adapters.<Feature>` per demo — port implementations, stage systems, views.
- `Client.Bootstrap` — `EntryPoint`, the composition root.

Reference direction rules:

- A feature assembly references only its shared tier: `Simulation.<F>` → `Simulation.Shared`;
  `Adapters.<F>` → `Simulation.Shared` + `Simulation.<F>` + `Adapters.Shared`. Never another
  feature.
...
- A cross-feature need is restructured (move the type to Shared, fix the direction) — never
...
- Views (MonoBehaviour) never touch ECS: no `EcsWorld`, pools, or command writes. A view
  raises a plain C# event; a system turns it into a command entity. Exception: `EntryPoint`.
- Async ports are handle-and-poll (`int Begin…`, `Poll(id)`, `Release(id)`); no `Task`,
  callbacks, or exceptions across the simulation boundary. Request ids and asset handle ids
  are different lifetimes — see ARCHITECTURE-GUIDE.md §5.
