...
namespace Client.Simulation.MagicWords.Systems
{
    ...
    /// </summary>
    internal sealed class DialogueIngestSimSystem : IEcsSim, IEcsInit,
        ...
        IEcsInject<ILogService>
    {
        ...
        public DialogueIngestSimSystem(MagicWordsConfig config)
        {
            _config = config;
        }
        ...
        /// <remarks>
        /// Reads and does not delete: the event is one frame long and <c>DialogueClnSystem</c>
        /// owns its end. Only the first is taken, because <c>DialogueFetchSimSystem</c> raises one per
