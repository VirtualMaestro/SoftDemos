...
namespace Client.Adapters.Tests
{
    public sealed class AceOfShadowsPresentationTests
    {
        ...
        public IEnumerator ReopeningTheDemo_LoadsOnce()
        {
            ...
            // and the background — not the card back or the 13 faces cut out of the atlas.
            ...
            for (var pass = 1; pass <= 2; pass++)
            {
                ...
                yield return _WaitUntil(() => boot.World.GetPool<DemoReadyTag>().Count > 0,
                    $"Ace of Shadows never became ready on open {pass}.", LoadTimeoutSeconds);
                ...
                Assert.That(boot.Views.Count, Is.Zero,
                    $"Close {pass} must hand every card view back to the registry.");
            }
